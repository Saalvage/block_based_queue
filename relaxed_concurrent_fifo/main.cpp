...
static std::pair<std::uint64_t, std::uint32_t> sequential_bfs(
    const Graph& graph) {
    multififo::RingBuffer<std::uint32_t> nodes(make_po2(graph.num_nodes()));
    std::vector<std::uint32_t> distances(
        graph.num_nodes(), std::numeric_limits<std::uint32_t>::max());
    distances[0] = 0;
...
void test_consistency(std::size_t fifo_size, std::size_t elements_per_thread,
                      double prefill) {
    bbq_min_block_count<std::uint64_t, THREAD_COUNT * BLOCK_MULTIPLIER> fifo{
        THREAD_COUNT, fifo_size};
...
void test_continuous_bitset_claim() {
    ...
    while (true) {
        ...
        auto result =
            a.template claim_bit<claim_value::ONE, claim_mode::READ_WRITE>();
...
template <typename BENCHMARK, typename BENCHMARK_DATA_TYPE = benchmark_info,
          typename... Args>
void run_benchmark(
    const std::string& test_name,
    const std::vector<std::unique_ptr<benchmark_provider<BENCHMARK>>>&
        instances,
    double prefill, const std::vector<int>& processor_counts,
    int test_iterations, int test_time_seconds, const Args&... args) {
    constexpr const char* format = "fifo-{}-{}-{:%FT%H-%M-%S}.csv";

    if (BENCHMARK::HAS_TIMEOUT) {
        std::cout << "Expected running time: ";
        auto running_time_seconds = test_iterations * test_time_seconds *
                                    processor_counts.size() * instances.size();
...
int main(int argc, const char** argv) {
    ...
    for (int i = 1; i <= static_cast<int>(std::thread::hardware_concurrency());
         i *= 2) {
    ...
    if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
        std::cout << "Usage: " << argv[0]
                  << " <experiment_no> <graph_file>? [-h | --help] "
                     "[-t | --thread_count <count>] "
                     "[-s | --test_time_seconds <count> (default "
                  << TEST_TIME_SECONDS_DEFAULT
                  << ")] "
                     "[-r | --run_count <count> (default "
    ...
    for (int i = input >= 7 ? 3 : 2; i < argc; i++) {
        if (strcmp(argv[i], "-t") == 0 ||
            ...
            if (is_exclude) {
                if (!fifo_set.empty()) {
                    std::cerr << "Cannot specify -i and -e at the same time!"
                              << std::endl;
                    return 1;
                }
                is_exclude = false;
            ...
            std::cerr << std::format("Unknown argument \"{}\"!", argv[i])
                      << std::endl;
            return 1;
        }
    }
    ...
    switch (input) {
        case 1: {
            std::vector<std::unique_ptr<benchmark_provider<benchmark_default>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark("comp", instances, prefill_override.value_or(0.5),
                          processor_counts, test_its, test_time_secs);
        } break;
        case 2: {
            std::vector<
                std::unique_ptr<benchmark_provider<benchmark_quality<>>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark("quality", instances, prefill_override.value_or(0.5),
                          processor_counts, test_its, test_time_secs);
        } break;
        case 3: {
            std::vector<
                std::unique_ptr<benchmark_provider<benchmark_quality<true>>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark("quality-max", instances,
                          prefill_override.value_or(0.5),
                          {processor_counts.back()}, 1, test_time_secs);
        } break;
        case 4: {
            std::vector<std::unique_ptr<benchmark_provider<benchmark_fill>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark("fill", instances, prefill_override.value_or(0),
                          processor_counts, test_its, 10);
        } break;
        case 5: {
            std::vector<std::unique_ptr<benchmark_provider<benchmark_empty>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark("empty", instances, prefill_override.value_or(1),
                          processor_counts, test_its, 10);
        } break;
        case 6: {
            std::vector<std::unique_ptr<benchmark_provider<benchmark_prodcon>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            // TODO: This can be done nicer to account for different thread
            // counts.
            for (int producers = 8; producers < 128; producers += 8) {
                auto consumers = 128 - producers;
                run_benchmark<benchmark_prodcon, benchmark_info_prodcon, int,
                              int>(
                    std::format("prodcon-{}-{}", producers, consumers),
                    instances, prefill_override.value_or(0.5),
                    {processor_counts.back()}, test_its, test_time_secs,
                    producers, consumers);
            }
        } break;
        case 7: {
            ...
            std::vector<std::unique_ptr<benchmark_provider<benchmark_bfs>>>
                instances;
            add_instances(instances, fifo_set, is_exclude);
            run_benchmark<benchmark_bfs, benchmark_info_graph, Graph*>(
                std::format("bfs-{}", graph_file.filename().string()),
