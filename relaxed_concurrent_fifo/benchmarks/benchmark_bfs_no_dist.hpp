#ifndef BENCHMARK_GRAPH_NO_DIST_HPP_INCLUDED
#define BENCHMARK_GRAPH_NO_DIST_HPP_INCLUDED
...
struct benchmark_bfs_no_dist : benchmark_timed<> {
    struct Counter {
...
#pragma GCC diagnostic pop
    ...
    benchmark_bfs_no_dist(const benchmark_info& info)
    ...
    void process_node(std::uint64_t node, typename FIFO::handle& handle,
                      Counter& counter) {
        auto current_distance =
            distances[node].value.load(std::memory_order_relaxed);
        do {
    ...
    template <typename FIFO>
    void per_thread(int thread_index, typename FIFO::handle& handle,
                    std::barrier<>& a) {
        Counter counter;
        if (thread_index == 0) {
            distances[0].value = 1;
