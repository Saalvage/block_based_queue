...
static inline int  zipf(double alpha, const int max)
{
 ...
 */
static inline struct zipf_arr* zipf_get_rand_array(double zipf_alpha,
		    size_t num_vals,
		    const int max,
		    const int id)
{
...
static inline int zipf_get_next(struct zipf_arr* za)
{
  return za->vals[(za->i++) % za->size];
}

static inline void zipf_print_stats(struct zipf_arr* za)
{
#if ZIPF_STATS == 1
  int i;
  for (i = 0; i < za->max; i++)
