...
public:
	...
	public:
		bool push(T t) {
			...
			std::uint64_t index;
			bool claimed = false;
			while (!epoch_valid(get_epoch(ei), mask_epoch(write_epoch)) || (index = get_write_index(ei)) == CELLS_PER_BLOCK || !header->epoch_and_indices.compare_exchange_weak(ei, ei + 1, std::memory_order_relaxed)) {
		...
		std::optional<T> pop() {
			...
			while (!epoch_valid(get_epoch(ei), mask_epoch(read_epoch)) || (index = get_read_started_index(ei)) == get_write_index(ei)
