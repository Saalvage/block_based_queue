...
struct window {
	alignas(std::hardware_destructive_interference_size) atomic_bitset<BLOCKS_PER_WINDOW, BITSET_T> touched_set;
	...
	BLOCK_T* try_get_write_block(int starting_bit) {
		std::size_t free_bit = filled_set.template claim_bit<claim_value::ZERO, claim_mode::READ_WRITE>(starting_bit, std::memory_order_relaxed);
		...
		touched_set.set(free_bit, std::memory_order_relaxed);
		return &blocks[free_bit];
	}

	BLOCK_T* try_get_free_read_block(int starting_bit) {
		std::size_t free_bit = touched_set.template claim_bit<claim_value::ONE, claim_mode::READ_WRITE>(starting_bit, std::memory_order_relaxed);
		if (free_bit == std::numeric_limits<std::size_t>::max()) {
			return nullptr;
		}
		return &blocks[free_bit];
	...
	BLOCK_T* try_get_any_read_block(int starting_bit) {
		std::size_t free_bit = filled_set.template claim_bit<claim_value::ONE, claim_mode::READ_ONLY>(starting_bit, std::memory_order_relaxed);
		if (free_bit == std::numeric_limits<std::size_t>::max()) {
			return nullptr;
		}
		return &blocks[free_bit];
...
public:
	...
	private:
		...
		bool claim_new_block_read() {
			...
			do {
				...
				window_t& window = fifo.index_to_window(window_index);
				new_block = window.try_get_free_read_block(random_bit_index());
				if (new_block == nullptr) {
					if (is_ahead) {
						dont_advance = true;
						continue;
					...
					new_block = window.try_get_any_read_block(random_bit_index());
					if (new_block != nullptr) {
						break;
					}
	...
	public:
		bool push(T t) {
			...
			std::uint64_t index;
			bool claimed = false;
			while (get_epoch(ei) != mask_epoch(write_epoch) || (index = get_write_index(ei)) == CELLS_PER_BLOCK || !header->epoch_and_indices.compare_exchange_weak(ei, ei + 1, std::memory_order_relaxed)) {
		...
		std::optional<T> pop() {
			...
			while (get_epoch(ei) != mask_epoch(read_epoch) || (index = get_read_started_index(ei)) == get_write_index(ei)
