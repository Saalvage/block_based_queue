...
public:
	...
	block_based_queue([[maybe_unused]] int thread_count, std::size_t min_size) :
		...
		for (std::size_t j = 0; j < blocks_per_window; j++) {
			window.filled_set.set_epoch_if_empty(0, 1);
			...
			do {
				...
				if (new_block == nullptr) {
					...
					if (write_window == window_index + 1) {
						...
						new_window.filled_set.set_epoch_if_empty(fifo.window_to_epoch(write_window), fifo.window_to_epoch(write_window) + 1, std::memory_order_relaxed);
