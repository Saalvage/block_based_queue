...
private:
    ...
    static constexpr std::size_t arr_type_mask = (1 << bit_count) - 1;
    ...
    static constexpr std::uint64_t get_bits(std::uint64_t epoch_and_bits) { return epoch_and_bits & arr_type_mask; }
...
public:
	...
	constexpr void set_all(std::size_t window_index, std::uint64_t epoch, std::memory_order order = BITSET_DEFAULT_MEMORY_ORDER) {
        for (std::size_t i = 0; i < blocks_per_window; i++) {
            auto& elem = data[window_index * blocks_per_window + i];
            std::uint64_t eb = elem->load(order);
            // Test-test-set
            while (get_epoch(eb) == epoch && get_bits(eb) != arr_type_mask && !elem->compare_exchange_weak(eb, eb | arr_type_mask, order));
        }
    }
