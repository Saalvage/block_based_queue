...
static void add_instances(std::vector<std::unique_ptr<benchmark_provider<BENCHMARK>>>& instances, bool parameter_tuning, std::unordered_set<std::string>& filter_set, bool are_exclude_filters) {
	...
	if (parameter_tuning) {
		for (double b = 0.125; b <= 16; b *= 2) {
			for (int c = 2; c <= 2048; c *= 2) {
				instances.push_back(std::make_unique<benchmark_provider_bbq<BENCHMARK>>("{},{},blockfifo", b, c - 1));
			}
		}
	} else {
		instances.push_back(std::make_unique<benchmark_provider_bbq<BENCHMARK>>("blockfifo-{}-{}", 2, 1));
		instances.push_back(std::make_unique<benchmark_provider_bbq<BENCHMARK>>("blockfifo-{}-{}", 4, 15));
		instances.push_back(std::make_unique<benchmark_provider_bbq<BENCHMARK>>("blockfifo-{}-{}", 4, 255));
	}
#endif
