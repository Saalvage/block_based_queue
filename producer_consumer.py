...
threads_raw = sys.argv[1] if len(sys.argv) >= 2 else input(f"Please enter the thread count ({os.cpu_count()}): ")
threads = os.cpu_count() if threads_raw == "" else int(threads_raw)
out_dir = sys.argv[2] if len(sys.argv) >= 3 else "."

step = int(threads / 16)

with open(f"{out_dir}/producer-consumer-{threads}.csv", "w") as out:
    i = step
    while i < threads:
